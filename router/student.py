...
@router.get("/student")
def get_students(
    current_admin: Annotated[Admin, Depends(get_current_user)], session: SessionDep
):
...
@router.post("/student")
def create_sudent(
    current_admin: Annotated[Admin, Depends(get_current_user)],
    request: StudenCreate,
    session: SessionDep,
):
...
def update_sutdent(
    ...
    if not student:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Student is not in Database"
        )
