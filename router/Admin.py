...
from module import AdminCreate,Admin,AdminPublic
...
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()],session:SessionDep) -> Token:
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"{form_data.username} username not found ")
